...
const connectDB = async (): Promise<typeof mongoose> => {
  ...
  // Connection-level event listeners (fire after the initial handshake).
  ...
  try {
    const conn = await mongoose.connect(uri, {
      serverSelectionTimeoutMS: 10_000, // fail fast if cluster is paused / unreachable
    ...
    // Categorize the error so you know exactly what to fix.
    ...
    // Re-throw so the caller (server.ts) decides whether to exit.
    throw err;
  }
};
